...
while ($row2 = $query2->fetch_assoc()) {
	if (strtotime($row2['date']) < $datetod) {
		...
		// Delete cache file
		if(is_file('modules/cache/ip-details/' . $row2['ip'] . '.json')) {
			unlink('modules/cache/ip-details/' . $row2['ip'] . '.json'); 
		}
	}
}
...
if (isset($_GET['disable'])) {
	...
	$files = glob('modules/cache/ip-details/*'); // Get all cache file names
